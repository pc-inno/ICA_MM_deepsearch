...
Provides HTTP/SSE based API server using FastMCP library.
"""
...
class FastMCPAdapter:
        ...
        try:
            ...
            async def tool_wrapper(**kwargs):
                ...
                try:
                    ...
                    self.logger.debug(f"Tool '{tool.name}' parameters with {kwargs} executed with result: {result}")
                    # Format response
                    if result['success']:
    ...
    def run_sync(self, host: str = None, port: int = None):
        ...
        try:
            ...
            if self.transport == "stdio":
                self.mcp.run(transport="stdio", log_level="warning")
            else:
                self.mcp.run(transport=self.transport, host=host, port=port, log_level="warning")
